...
# ==================== TRANSFORMATION PROCESSING FUNCTIONS ====================
    ...
    for parent_key, trans_group in competing_transformations.items():
        ...
        for trans_info in trans_group:
            ...
            if trans_type == "DIFFUSIONAL" and input_option in ("JMA", "TTT"):
        ...
        # Now process active competing transformations 
        if len(active_transformations) > 0:                
            ...
            for trans in active_transformations:                
                ...
                if f_p_prev > ((f_total*f_eq_parent) + tolerance):                     
                    ...
                    if VERBOSE: print(f"Parent < f_eq: trans_name: {trans_name},f_eq_child: {f_eq_child}, f_c_prev: {f_c_prev}, f_p: {f_p}, f_prev_parent: {f_p_prev}")
    ...
    """Process transformations sequentially (Ti-6Al-4V style)."""
    
    for t_rule in trans_rules:
        
        # Skip empty or incomplete rules - ensure we have minimum required elements
        if len(t_rule) < 12:  # Need at least indices 0-11 for basic functionality
            continue
            
        # Number of parent/child phases is defined in t_rule[10] and t_rule[11]
        nparent = int(t_rule[10]) if len(t_rule) > 10 and t_rule[10] is not None else 1
        nchild = int(t_rule[11]) if len(t_rule) > 11 and t_rule[11] is not None else 1
        
        # Extract parent and child names with bounds checking
        parent_names = []
        child_names = []
        
        # Parent names start at index 12        
        for j in range(nparent):
            idx = 12 + j
            if len(t_rule) > idx and t_rule[idx] is not None and t_rule[idx].strip():
                parent_names.append(t_rule[idx])
        #if VERBOSE: print("Parent names:", parent_names)

        # Child names start at index 15 (assuming max 3 parents)
        for j in range(nchild):
            idx = 15 + j
            if len(t_rule) > idx and t_rule[idx] is not None and t_rule[idx].strip():
                child_names.append(t_rule[idx])
        #if VERBOSE: print("Child names:", child_names)
        ...
        # All other triggers and options with bounds checking
        trans_name = t_rule[0] if len(t_rule) > 0 and t_rule[0] else "Unknown"
        Trate_cond = t_rule[1] if len(t_rule) > 1 and t_rule[1] else "NONE"
        Trate_lo = t_rule[2] if len(t_rule) > 2 and t_rule[2] is not None else -1e6
        Trate_hi = t_rule[3] if len(t_rule) > 3 and t_rule[3] is not None else 1e6
